function in_rain_area(x, y)
    local tile_x = flr(x / 8)
    local tile_y = flr(y / 8)

    for area in all(rain_areas) do
        if tile_x >= area.x1 and tile_x <= area.x2 and
           tile_y >= area.y1 and tile_y <= area.y2 then
            return true
        end
    end
    return false
...
function init_rain()
    ...
    for i=1,rain_count do
        local is_far = rnd() < 0.4  -- 40% far/small drops
        add(rain, {
            x = rnd(128),
            y = rnd(128) - 128,
            spd = is_far and (1.5 + rnd(1)) or (2.5 + rnd(1.5)),
            len = is_far and (2 + rnd(1)) or (3 + rnd(3)),
            far = is_far
        })
    end
    rain_active = true
...
function update_rain()
    local player_in_rain = in_rain_area(p.x, p.y)

    if player_in_rain and not rain_active then
        init_rain()
    elseif not player_in_rain and rain_active then
        rain_active = false
        rain = {}
    end
    ...
    if rain_active then
        for r in all(rain) do
            ...
            if r.y > cam_y + 128 then
                r.y = cam_y - 8
                r.x = cam_x + rnd(128)
            end

            if r.x < cam_x then r.x = cam_x end
            if r.x > cam_x + 127 then r.x = cam_x + 127 end
        end
    end
end
